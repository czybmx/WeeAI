...
def generate_reply(user_input, history_manager):
    ...
    # 判断是否需要进行网络搜索
    needs_web_search = ask_if_needs_web_search(user_input, "", recent_history)  # 此时还未生成assistant_response，因此传入空字符串
    if needs_web_search.strip().upper() == "YES":
        # 通知用户将进行网络搜索
        yield "\n\n*Performing a web search to provide a better answer...*\n\n"
        
        # 获取搜索关键词
        search_keyword = ask_for_search_keyword(user_input, "", recent_history)

        if search_keyword:
            # 执行搜索
            search_results = perform_searches(search_keyword)
            # 构建包含搜索结果的提示
            search_results_output = f"(Search results for '{search_keyword}':\n{search_results})\n"

            final_prompt = f"""【Conversation History】\n{context}\nUser: {user_input}\n{search_results_output}\nPlease provide a detailed answer to the user's question, using the information from the search results when appropriate."""
...
""",
            ...
            yield "Unable to determine the search keyword."
...
def ask_if_needs_web_search(user_input, assistant_response, recent_history):
    context = "".join(f"User: {record['user']}\nAI: {record['response']}\n" for record in recent_history)

    private_prompt = f"""【Conversation History】\n{context}\nUser: {user_input}\nAssistant: {assistant_response}\nDetermine whether the assistant needs to perform a web search to answer the user's question. JUST ANSWER YES OR NO, NO NEED EXPLAIN."""

    data = {
        "model": MODEL_NAME,
        "temperature": 0.5,
        "system": """You are a Web Search Assistant. Your task is to determine if the assistant needs to perform a web search to answer the user's question. Analyze the user's input and the assistant's response, and determine if a web search is needed. JUST ANSWER YES OR NO, NO NEED EXPLAIN.""",
...
# Function to extract search keywords
def ask_for_search_keyword(user_input, assistant_response, recent_history):
    context = "".join(f"User: {record['user']}\nAI: {record['response']}\n" for record in recent_history)

    private_prompt = f"""【Conversation History】\n{context}\nUser: {user_input}\nAssistant: {assistant_response}\nPlease extract the KEYWORDS or PHRASE for a web search that would help answer the user's question. JUST RETURN THE KEYWORDS, NO NEED TO EXPLAIN."""

    data = {
        "model": MODEL_NAME,
        "temperature": 0.5,
        "system": """You are a Search Keyword Extractor. Your task is to extract the most relevant keywords or phrase for a web search based on the user's question and the assistant's response. Do not include any additional information. JUST RETURN THE KEYWORDS, NO NEED TO EXPLAIN.""",
        "prompt": private_prompt,
        "stream": False
    }
    try:
...
# Function to perform Google search and fetch content
def perform_searches(query):
    ...
    search_results = f"Search result URLs:\n" + "\n".join(google_results)
